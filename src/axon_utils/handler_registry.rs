...
pub(crate) type Handler<'a, T, P, R> = &'a (dyn Fn(T, P) -> Pin<Box<SendFtr<R>>> + Sync);
...
pub trait HandlerRegistry<P, W>: Send {
    ...
    fn insert<T: Send + Clone>(
        ...
        handler: Handler<'static, T, P, ()>,
    ) -> Result<()>;
    fn insert_ignoring_output<T: Send + Clone, R: Clone>(
        &mut self,
        name: &str,
        deserializer: Deserializer<'static, T>,
        handler: Handler<'static, T, P, Option<R>>,
    ) -> Result<()>;
    fn insert_with_output<T: Send + Clone>(
        &mut self,
        name: &str,
        deserializer: Deserializer<'static, T>,
        handler: Handler<'static, T, P, Option<W>>,
    ) -> Result<()>;
    ...
    fn get(&self, name: &str) -> Option<&dyn SubscriptionHandle<P, W>>;
}
...
struct Subscription<'a, P, T, R, W> {
    ...
    pub handler: Handler<'a, T, P, Option<R>>,
